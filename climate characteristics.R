...
#temp yearly####
yearly_mn_temp = mt_mn_temp %>% 
  group_by(gauge, year(yr_mt)) %>% 
  summarise(mean_temp = mean(temp_m)) %>% 
  spread(key=gauge, value=mean_temp) %>% 
  as.data.frame()

temp_30_ave = rollapply(data = da_temp_wide,FUN= mean, width=30, by.column=TRUE, fill=NA, align="center")

yearly_max_temp = temp_30_ave %>% 
  group_by(gauge, year(yr_mt)) %>% 
  summarise(mean_temp = max(temp_m)) %>% 
  spread(key=gauge, value=mean_temp) %>% 
  as.data.frame()
